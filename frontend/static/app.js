...
// ---- Gemini API key (load from gemini.txt if present; fallback to hardcoded) ----
let API_KEY = 'YOUR_API_KEY';
(async () => {
  try {
    const res = await fetch('./gemini.txt', { cache: 'no-store' });
    if (res.ok) {
      const txt = (await res.text()).trim();
      if (txt) API_KEY = txt;
    }
  } catch {
    // Couldn't read gemini.txt; keep fallback API_KEY
  }
})();
// -------------------------------------------------------------------------------
...
const DATASET_PROMPTS = {
    "NE_countries.geojson": [
      ...
      "Show France in red and all other countries in blue",
        "Show Russia in green and Brazil in red and all other countries in blue"
    ], "Roads.geojson": [
        ...
        "Show roads longer than 50 km and shorter than 200 km in red, and toll roads in blue"
                ...
                <div style={{display: 'grid', gridTemplateColumns: 'repeat(3, minmax(0,1fr))', gap: 6}}>
                    <select
                        ...
                        onChange={(e) => {
                            ...
                            setH(v);
                            onChange(joinDateTime(selDate, v, M, S, true));
                        }}
                    ...
                    <select
                        ...
                        onChange={(e) => {
                            const v = +e.target.value;
                            setM(v);
                            onChange(joinDateTime(selDate, H, v, S, true));
                        }}
                    ...
                    <select
                        ...
                        onChange={(e) => {
                            const v = +e.target.value;
                            setS(v);
                            onChange(joinDateTime(selDate, H, M, v, true));
                        }}
...
function App() {
    ...
    useEffect(() => {
        ...
        setAiError("");
        setAutoError("");
        setColorConf(c => ({...c, attribute: "", type: "basic", fillColor: "#2b6cb0", rules: null}));
    ...
    const saveEnhancedToCache = async (name, payload) => {
        try {
            ...
            console.warn("saveEnhancedToCache failed", e);
        }
    };
    ...
    const saveAutoStyleToCache = async (name, suggestions) => {
        try {
            ...
            console.warn("saveAutoStyleToCache failed", e);
        }
    };
        ...
        const run = () => {
            ...
            for (let i = 0; i < raw.length; i++) {
                ...
                if (typeof v === 'string') {
                    ...
                    if (match) {
                        hasH = true;
                        hasM = match[2] !== undefined;
                        hasS = match[3] !== undefined;
                    } else if (v.includes(':')) {
                        hasH = true;
                        hasM = true;
                    }
    ...
    useEffect(() => {
        ...
        labelOnlyCache.current.clear();
        pointLabelCache.current.clear();
    }, [labelConf.attribute, labelConf.fillColor, labelConf.strokeColor]);
...
const styleProducts = useMemo(() => {
  ...
  // ✅ 1) RULES FIRST — build legend and return immediately
  if (type === 'rules' && Array.isArray(rules) && rules.length) {
    const items = rules.map((r) => ({
      label: describeRule(r),              // e.g. "CONTINENT == Europe" or provided legend
      color: r.fill || '#2b6cb0'
    }));
    legend = { title: 'Rule-based styling', items };
    return { cache, legend };              // cache not needed for rules
  }
  ...
  if (type === 'categorized' && attribute) {
    ...
    sorted.forEach(({ v }, i) => {
      cache[v] = (catColors && catColors[v]) ? catColors[v] : fallback(i);
    });
  ...
  // 3) GRADUATED (equal-width 10 bins)
  if (type === 'graduated' && attribute) {
    const vals = feats
    ...
    // fall through to basic if no numeric data
  }
  ...
  return { cache, legend };
    ...
    useEffect(() => {
        ...
        const {attribute, type, gradStart, gradEnd, rules} = colorConf;
        ...
        const styleFn = (feature, resolution) => {
            ...
            if (type === 'rules' && Array.isArray(rules) && rules.length) {
                ...
                for (const r of rules) {
                    if (!r.else && testRule(feature, r)) {
                        hit = r;
                        break;
                    }
                }
                if (!hit) hit = rules.find(r => r.else) || null;

                const fill = (hit?.fill) || '#2b6cb0';
                const stroke = (hit?.stroke) || strokeColor;
                const baseStyle = getStyleFor(feature, fill, stroke);

                // labels still applied if enabled
                return labelConf.enabled && labelConf.attribute
                    ? [baseStyle, getLabelOnlyStyle(String(feature.get(labelConf.attribute)), labelConf.fillColor, labelConf.strokeColor)]
                    : baseStyle;
            }
    ...
    /* ─── Handlers ──────────────────────────────────────────────────────────── */
    const applyColor = (conf) => {
        setColorConf(c => ({...c, ...conf}));
        clearAllStyleCaches();
    };
    ...
    // Build a strict, self-contained instruction for rule-based requests
    const buildRulesHint = (schema) => {
        const fieldList = Array.isArray(schema) ? schema.map(f => f.name).join(', ') : '(unknown fields)';
        return [
            'You are a GIS styling generator. Return ONLY JSON.',
            'Output schema: {"suggestions":[ ... ]}. Each suggestion has keys:',
            '- type: one of "basic","categorized","graduated","label","rules"',
            '- attribute, fillColor, strokeColor, explanation (optional depending on type)',
            '- For "rules": {"type":"rules","rules":[{ "when":{"all":[{"attr":"<field>","op":"==","value":"<literal>"}]}, "fill":"#RRGGBB","stroke":"#RRGGBB","legend":"<text>" }, { "else":true, "fill":"#RRGGBB","stroke":"#RRGGBB","legend":"Other"} ]}',
            'CRITICAL: If the user asks to color a subset vs the rest (e.g., "countries in Europe one color, others another"), you MUST return a single suggestion with "type":"rules". Do NOT use "categorized" for this case.',
            `Dataset fields you may reference exactly: ${fieldList}`,
            'Allowed ops: ==, !=, in, not_in, >, >=, <, <=, contains.',
        ].join('\n');
    };

    const RULES_HINT =
        ' Return a JSON object {suggestions:[...]} where a suggestion MAY be {"type":"rules","rules":[{ "when":{"all":[{"attr":"continent","op":"==","value":"Europe"}]}, "fill":"#3B82F6","stroke":"#333333","legend":"Europe"}, {"else":true,"fill":"#E5E7EB","stroke":"#333333","legend":"Other"} ]}. Allowed ops: ==, !=, in, not_in, >, >=, <, <=, contains. Use exact field names from the dataset.';
    ...
    const normalizeSuggestion = (s) => {
        ...
        const type = (s.type || s.style || "basic").toLowerCase();
        const rules = normalizeRules(s.rules || []);
        return {
            ...
            rules: (type === 'rules' && rules && rules.length) ? rules : null
        };
    };
    ...
    const normOp = (op) => {
        ...
        if (['==', 'eq', 'equals', 'is'].includes(s)) return '==';
        if (['!=', 'ne', 'neq', 'not_equals'].includes(s)) return '!=';
        if (['>', 'gt'].includes(s)) return '>';
        if (['>=', 'gte'].includes(s)) return '>=';
        if (['<', 'lt'].includes(s)) return '<';
        if (['<=', 'lte'].includes(s)) return '<=';
        if (['in'].includes(s)) return 'in';
        if (['not_in', 'nin'].includes(s)) return 'not_in';
        if (['contains', 'has', 'includes'].includes(s)) return 'contains';
        return s;
    };
    ...
    const normalizeRules = (rules) => {
        ...
        const normCond = (c) => {
            ...
            const attr = c.attr || c.attribute || c.field || '';
            const op = normOp(c.op);
            let values = undefined;
            if (Array.isArray(c.values)) values = c.values;
            else if ('value' in c) values = Array.isArray(c.value) ? c.value : [c.value];
            else if ('eq' in c) values = [c.eq];
            return {attr, op, values};
        };
        const normWhen = (w) => {
            if (!w || typeof w !== 'object') return null;
            if (Array.isArray(w.all)) return {all: w.all.map(normCond).filter(Boolean)};
            if (Array.isArray(w.any)) return {any: w.any.map(normCond).filter(Boolean)};
            const one = normCond(w); // shorthand
            return one ? {all: [one]} : null;
        };
        return rules.map(r => {
            const when = r.else ? null : normWhen(r.when || r.if || r.condition);
            const fill = r.fill || r.fillColor || r.color || '#2b6cb0';
            const stroke = r.stroke || r.strokeColor || '#333333';
            const legend = r.legend || r.label || null;
            const isElse = !!r.else || !!r.default;
            return {when, else: isElse, fill, stroke, legend};
        }).filter(r => r.else || r.when);
    };
    ...
    const askAI = async () => {
        ...
        setLlmLoading(true);
        try {
            const composed = `${prompt}\n\n${buildRulesHint(schema)}`;
            const res = await fetch(`/datasets/${selected}/style.json?prompt=${encodeURIComponent(composed)}`);
            //const raw2 = res.json();
            //console.log("LLM raw response:", raw2);   // 👈 see exactly what came back


            const maybeText = await res.text();
            let payload;
            try {
                payload = JSON.parse(maybeText);
            } catch {
                payload = maybeText;
            }
            ...
            if (arr.length) {
                ...
                if (s.type === 'label') {
                    applyLabel(s);
                } else if (s.type === 'rules' && s.rules) {
                    applyColor({type: 'rules', rules: s.rules});
                } else {
                    applyColor({
    ...
    const askAuto = async () => {
        ...
        const instCore = "Instruction: For each attribute, suggest styling config as a JSON object list under key suggestions. Each item has keys {attribute,type,explanation}. Allowed types: basic, categorized, graduated, label, rules. Keep it concise.";
        const inst = `${instCore}\n${RULES_HINT}`;
        ...
        try {
            ...
            openModalWith(used);
            try {
                if (used.length) await saveAutoStyleToCache(selected, used);
            } catch {
            }
    ...
    const getAttributeSample = (attr, n = 5) => {
        ...
        if (Array.isArray(sample) && sample.length) {
            try {
                vals = sample.map(r => r?.[attr]).filter(v => v !== undefined && v !== null).slice(0, n);
            } catch {
            }
        }
...
function matchCond(featVal, {op, values}) {
  const vArr = values ?? [];
  const first = vArr[0];
  const asNum = (x) => (x === null || x === undefined || x === '') ? NaN : Number(x);
  const aNum = asNum(featVal), bNum = asNum(first);
  switch (op) {
    case '==':   return featVal === first;
    case '!=':   return featVal !== first;
    case '>':    return !isNaN(aNum) && !isNaN(bNum) && aNum > bNum;
    case '>=':   return !isNaN(aNum) && !isNaN(bNum) && aNum >= bNum;
    case '<':    return !isNaN(aNum) && !isNaN(bNum) && aNum < bNum;
    case '<=':   return !isNaN(aNum) && !isNaN(bNum) && aNum <= bNum;
    case 'in':   return vArr.some(x => x === featVal);
    case 'not_in': return !vArr.some(x => x === featVal);
    case 'contains':
      return (typeof featVal === 'string') && (typeof first === 'string') && featVal.includes(first);
    default:     return false;
  }
}

function testRule(feature, rule) {
  if (rule.else) return true;
  const w = rule.when;
  if (!w) return false;
  if (Array.isArray(w.all)) return w.all.every(c => c && c.attr && matchCond(feature.get(c.attr), c));
  if (Array.isArray(w.any)) return w.any.some(c => c && c.attr && matchCond(feature.get(c.attr), c));
  return false;
}

function describeRule(r) {
  if (r.legend) return r.legend;
  if (r.else) return 'Other';
  const parts = (r.when?.all || r.when?.any || []).map(c => `${c.attr} ${c.op} ${(c.values || []).join(',')}`);
                ...
                {autoSuggestions.length === 0 ? (
                    ...
                    autoSuggestions.map((s, i) => (
                        <div key={i} style={{border: '1px solid #eee', borderRadius: 6, padding: 8, marginBottom: 8}}>
                            ...
                            {s.explanation &&
                                <div style={{color: '#555', fontSize: 12, margin: '4px 0 6px'}}>{s.explanation}</div>}
                            <div style={{display: 'flex', gap: 8}}>
                                {s.type === 'label'
                                    ? <button onClick={() => applyLabel({attribute: s.attribute})}>Apply label</button>
                                    : s.type === 'rules' && s.rules
                                        ? <button onClick={() => applyColor({type: 'rules', rules: s.rules})}>Apply
                                            rules</button>
            ...
            <div className="sidebar" style={{
                ...
                {!selected ? (
                    ...
                    <select value={prompt} onChange={e => setPrompt(e.target.value)}
                        ...
                        {(examplePrompts || DEFAULT_PROMPTS).map((p, idx) => (
                ...
                <textarea rows={3} style={{width: '100%'}} placeholder="Describe what styling you want…" value={prompt}
                          onChange={e => setPrompt(e.target.value)}/>
                <div style={{display: 'flex', gap: 6, alignItems: 'center', marginTop: 6}}>
                    <button onClick={askAI} disabled={llmLoading}>{llmLoading ? "Loading…" : "Ask LLM"}</button>
                    <button onClick={askAuto}
                            disabled={autoLoading}>{autoLoading ? "Loading…" : "Suggest Attribute Styles"}</button>
                </div>
                ...
                <div style={{display: 'flex', gap: 6, alignItems: 'center'}}>
                    <label style={{display: 'flex', alignItems: 'center', gap: 6, whiteSpace: 'nowrap'}}>
                        <input type="checkbox" checked={labelConf.enabled}
                               onChange={e => setLabelConf(c => ({...c, enabled: e.target.checked}))}/>
                        Show
                    </label>
                    <select disabled={!labelConf.enabled} value={labelConf.attribute}
                            onChange={e => setLabelConf(c => ({...c, attribute: e.target.value}))} style={{flex: 1}}>
                            ...
                            <>
                                <div style={{
                                    ...
                                    {timeRanges[datetimeFilter.attribute]?.minFull
                                        ? <>Valid
                                            range: <b>{timeRanges[datetimeFilter.attribute].minFull}</b> — <b>{timeRanges[datetimeFilter.attribute].maxFull}</b></>
                    ...
                    <>
                        ...
                        {attrSample.name && (
                            <div style={{border: '1px solid #eee', borderRadius: 6, padding: 8, marginBottom: 8}}>
                                <div style={{fontWeight: 600, marginBottom: 4}}>Sample values
                                    for <code>{attrSample.name}</code></div>
                                {attrSample.values.length ? (
                                    <ul style={{margin: '0 0 0 16px', padding: 0}}>
                                        {attrSample.values.map((v, idx) => (
                                            <li key={idx} style={{margin: '2px 0'}}>{String(v)}</li>))}
                                    </ul>
                                ) : (
                ...
                <div style={{
                    ...
                    <div style={{
                        ...
                        {manual.styleType === 'basic' && (
                            <div style={{
                                ...
                                <div>
                                    ...
                                    <input type="color" value={manual.fillColor}
                                           onChange={e => setManual(m => ({...m, fillColor: e.target.value}))}
                                           style={colorBoxStyle}/>
                                </div>
                                <div>
                                    <div style={{fontSize: 12, marginBottom: 4}}>Stroke</div>
                                    <input type="color" value={manual.strokeColor}
                                           onChange={e => setManual(m => ({...m, strokeColor: e.target.value}))}
                                           style={colorBoxStyle}/>
                                </div>
                            </div>
                        )}
                        ...
                        {manual.styleType === 'categorized' && (
                            <>
                                {!manual.attribute &&
                                    <div style={{color: '#b00', marginBottom: 8}}>Pick an attribute first.</div>}
                                {manual.attribute && (
                                    <>
                                        <div style={{fontWeight: 600, margin: '8px 0 4px'}}>Top categories (edit
                                            colors)
                                        </div>
                                        <div style={{
                                            display: 'grid',
                                        ...
                                        <div style={{
                                            ...
                                            <div>
                                                ...
                                                <input type="color" value={manual.otherColor}
                        ...
                        {manual.styleType === 'graduated' && (
                            <>
                                {!manual.attribute &&
                                    <div style={{color: '#b00', marginBottom: 8}}>Pick a numeric attribute first.</div>}
                                <div style={{
                                    display: 'grid',
                                    gridTemplateColumns: '1fr 1fr',
                                    gap: 12,
                                    ...
                                    <div>
                                        ...
                                        <input type="color" value={manual.gradStart}
                                               onChange={e => setManual(m => ({...m, gradStart: e.target.value}))}
                                               style={colorBoxStyle}/>
                                    </div>
                                    <div>
                                        <div style={{fontSize: 12, marginBottom: 4}}>End (high)</div>
                                        <input type="color" value={manual.gradEnd}
                                               onChange={e => setManual(m => ({...m, gradEnd: e.target.value}))}
                                               style={colorBoxStyle}/>
                                    </div>
                                </div>
                        ...
                        {manual.styleType === 'label' && (
                            <>
                                ...
                                <select value={manual.labelAttribute}
                                        onChange={e => setManual(m => ({...m, labelAttribute: e.target.value}))}
                                        style={{width: '100%', marginBottom: 8}}>
                                    <option value="">— select —</option>
                                    {attributes.map(a => <option key={a} value={a}>{a}</option>)}
                                </select>
                                <div style={{
                                    ...
                                    <div>
                                        ...
                                        <input type="color" value={manual.labelFill}
                                               onChange={e => setManual(m => ({...m, labelFill: e.target.value}))}
                                               style={colorBoxStyle}/>
                                    </div>
                                    <div>
                                        <div style={{fontSize: 12, marginBottom: 4}}>Halo</div>
                                        <input type="color" value={manual.labelStroke}
                                               onChange={e => setManual(m => ({...m, labelStroke: e.target.value}))}
                                               style={colorBoxStyle}/>
                                    </div>
                                </div>
                            </>
                                ...
                                onClick={() => {
                                    if (manual.styleType === 'basic') {
                                        applyColor({
                                            ...
                                            strokeColor: manual.strokeColor
                                        });
                                        ...
                                        applyColor({
                                            ...
                                            strokeColor: manual.strokeColor,
                                            fillColor: "#eeeeee",
                                            catColors: manual.catColors || {},
                                            otherColor: manual.otherColor || "#cccccc",
                                        });
                                        ...
                                        applyLabel({
                                            attribute: manual.labelAttribute,
                                            fillColor: manual.labelFill,
                                            strokeColor: manual.labelStroke
                                        });
                                    }
                                    setModalOpen(false);
                                }}
            ...
            {schemaModalOpen && selectedField && (
                <div style={{
                    ...
                    <div style={{
                        ...
                        <div style={{fontSize: 12, color: '#555', marginBottom: 6}}>Type: <b>{selectedField.type}</b>
                        </div>
